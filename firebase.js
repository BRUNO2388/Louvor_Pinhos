...
Object.assign(window,{db,dbRef:ref,dbSet:set,dbRemove:remove,dbGet:get,dbUpdate:update,firebaseAuth:auth,firebaseSignIn:signInWithEmailAndPassword,firebaseSignOut:signOut});
let unsubReg=null,unsubMem=null,unsubAudit=null,unsubPerfis=null,unsubRole=null;
function iniciarLeituras(){
  [unsubReg,unsubMem,unsubAudit,unsubPerfis,unsubRole].forEach(u=>{if(u)u()});
...
onAuthStateChanged(auth,user=>{
  ...
  else{overlay.style.display='flex';bar.style.display='none';window.todosOsDados={};window.integrantesCadastrados={};window.roleLouvor='visualizacao';[unsubReg,unsubMem,unsubAudit,unsubPerfis,unsubRole].forEach(u=>{if(u)u()})}
});